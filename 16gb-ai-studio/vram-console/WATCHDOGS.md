...
> **最后更新**：2026-08-28
> **⚠️ 运行位置（2026-08-28 迁移）**：调度中心运行实例已从主仓库迁移至**工作区** `D:\Users\Danny\Documents\GMae_Amanda\16gb-ai-studio\vram-console`（与主仓库完全切割）。所有启停/看门狗均以工作区为准。
...
| 名称 | 类型 | 路径 | 状态 | 作用 | 启动方式 |
|------|------|------|------|------|---------|
| **GMae-Watchdog** | Python 脚本 | `vram-console/watchdog.py`（工作区） | ✅ 唯一看门狗 | 监控 server.py，崩溃 5 秒后自动重启 | 开机自启 / 手动 start.bat / 计划任务 |

### GMae-Watchdog 详情
- **脚本**：`D:\Users\Danny\Documents\GMae_Amanda\16gb-ai-studio\vram-console\watchdog.py`
- **启动器**：`run_watchdog.bat`（自动查找 python.exe 路径，已指向工作区）
- **隐藏启动**：`watchdog.vbs`（调用 run_watchdog.bat，隐藏窗口）
- **监控对象**：`server.py`（调度中心后端，端口 8787）
- **重启策略**：进程退出后等待 5 秒重启
- **防死循环**：1 小时内最多重启 10 次，超过则暂停 5 分钟
- **日志**：`logs/watchdog.log`；服务日志 `logs/vram-console.log`（按天轮转，保留 30 天）

### 临时持久启动手段（2026-08-28 迁移时使用）
| 名称 | 类型 | 指向 | 状态 | 说明 |
|------|------|------|------|------|
| **GMae_Watchdog_Oneshot** | Windows 计划任务（once） | 工作区 `watchdog.py`（WorkingDirectory=工作区，Doubao sandbox python） | ✅ 使用中 | 迁移时为使 watchdog 脱离会话持久运行而创建；不随开机自动触发（自启由 vbs 负责），无害保留 |

### 重要：沙箱环境限制说明
AI Agent 通过工具调用（Start-Process 等）启动的后台进程，在工具调用结束后可能被沙箱清理。
**正确启动方式**：
1. 用户手动双击工作区 `start.bat`（推荐）
2. 重启电脑，开机自启项自动生效（vbs 已指向工作区）
3. Agent 如需启动，用计划任务托管（如 GMae_Watchdog_Oneshot）或告知用户手动操作
...
| 名称 | 位置 | 指向 | 状态 |
|------|------|------|------|
| VRAM_Console.vbs | `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\` | `CurrentDirectory=工作区` + `run_watchdog.bat`（2026-08-28 已改指工作区） | ✅ 启用 |

> 开机后自动启动 GMae-Watchdog（从工作区），看门狗再启动 server.py。原 vbs 备份为 `VRAM_Console.vbs.bak_gmae_ws`。
...
## 八、2026-08-30 原系统看门狗停用确认

**主公确认**：原系统（家庭智能中枢）看门狗已停用，GMae 调度中心只用工作区 ram-console/watchdog.py。

- 不创建第二套看门狗
- 不修改原系统文件（完全切割原则）
- 工作区 watchdog.py 已升级为双探测（端口连通性 + /api/health 内容校验），防半死状态
- 开机自启项 VRAM_Console.vbs 已指向工作区
